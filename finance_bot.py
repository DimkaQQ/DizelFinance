...
INCOME_ARTICLES = {
    ...
    "Финансовая подушка":                "Движение активов",
    "Инвестиции / капитал":              "Движение активов",
    "Пенсия / долгий срок":              "Движение активов",
    "Дети и будущее детей":              "Движение активов",
    "Резерв на непредвиденные расходы":  "Движение активов",
    "Погашение кредитов":                "Движение активов",
    "Погашение ипотеки":                 "Движение активов",
    "Переезд / ремонт":                  "Движение активов",
    "Крупная покупка (техника, мебель)": "Движение активов",
    "Автомобиль / водительские расходы": "Движение активов",
    "Отпуск и путешествия":              "Движение активов",
    "Обучение и развитие":               "Движение активов",
    "Здоровье и медицина":               "Движение активов",
    "Подарки и праздники (цели)":        "Движение активов",
    "Прочие цели":                       "Движение активов",
}

# Расходы: Крупные расходы | Расходы | Долги
EXPENSE_ARTICLES = {
    # Крупные расходы
    "Аренда жилья":                      "Крупные расходы",
    "Детский сад / школа":               "Крупные расходы",
    "Подписки":                          "Крупные расходы",
    "Коммунальные платежи":              "Крупные расходы",
    "Связь и интернет":                  "Крупные расходы",
    "Страхование":                       "Крупные расходы",
    "Ипотека":                           "Крупные расходы",
    "Платеж по кредиту":                 "Крупные расходы",
    "Платеж по кредитной карте":         "Крупные расходы",
    "Налоги и сборы":                    "Крупные расходы",
    "Абонементы":                        "Крупные расходы",
    "Прочие обязательные платежи":       "Крупные расходы",
    # Расходы
    "Продукты":                          "Расходы",
    ...
    # ❌ Блок "Долги" полностью удалён
}

INCOME_BY_TABLE = {
    "Поступления": [a for a, t in INCOME_ARTICLES.items() if t == "Поступления"],
    "Движение активов": [a for a, t in INCOME_ARTICLES.items() if t == "Движение активов"],
}

EXPENSE_BY_TABLE = {
    "Крупные расходы": [a for a, t in EXPENSE_ARTICLES.items() if t == "Крупные расходы"],
...
CARDS            = ["Тинькофф", "Альфа", "Сбер", "Freedom", "Kaspi"]
...
TABLE_COLUMNS = {
    "Поступления":      (2,  12),   # B=название, L=факт
    "Крупные расходы":  (15, 25),   # O=название, Y=факт
    "Расходы":          (28, 38),   # AB=название, AL=факт
    "Движение активов": (54, 64),   # BB=название, BL=факт
    # ❌ "Долги": (41, 51) — удалён
}
...
def write_transaction_row(date_str: str, article: str, amount_rub: float,
                          currency: str, table_name: str, card: str, comment: str):
    sheet_name = get_month_sheet_name(date_str)
    try:
        ws = sh.worksheet(sheet_name)
    except Exception:
    ...
    # Доходы и Движение активов → колонки B, E, P
    is_income = table_name in ("Поступления", "Движение активов")  # ← добавлено
    if is_income:
    ...
    # Найти первую пустую строку в этом столбце начиная с 46
    try:
...
def table_choice_kb(tx_type: str):
    ...
    if tx_type == "Доход":
        kb.add("📥 Поступления")
        kb.add("🏦 Движение активов")  # ← было "Накопления"
    else:
...
TABLE_LABEL_MAP = {
    ...
    "🏦 Движение активов":"Движение активов",  # ← было "Накопления"
...
async def statistics(message: types.Message):
    try:
        ...
        icons = {
            "Поступления": "📥",
            "Движение активов": "🏦",  # ← было "Накопления"
            "Крупные расходы": "💳",
            "Расходы": "🛒",
            # "Долги" убран
        }
...
async def settings(message: types.Message):
    ...
    await message.answer(
        ...
        f"🤖 Gemini API: {'✅ '+GEMINI_MODEL if GEMINI_API_KEY else '❌ Не настроен'}\n"
        ...
        f"🛒 Расходы          → col AB / AL\n"
        f"🏦 Движение активов → col BB / BL",  # ← было "Накопления"
        parse_mode="HTML", reply_markup=main_menu_kb()
    )
...
if __name__ == "__main__":
    ...
    flask_thread.start()
    logging.info("🚀 DizelFinance Bot v2 запущен! (Gemini 2.5 Flash + Planergo Structure)")
    run_bot()